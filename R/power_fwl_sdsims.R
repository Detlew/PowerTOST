# ----------------------------------------------------------------------------
# simulate replicate design subject data and GCC rules
# ANOVA & average BE with fixed widened limits if CVwR>30%
#
# Author D. Labes based on power.scABEL.sds(), or better on 
# working horse .pwr.SABE.sds()
# 
...
# ----------------------------------------------------------------------------
power.fwl.sds <- function(alpha=0.05, theta1, theta2, theta0, CV, n,   
...
#  if (reg$est_method=="ISC") stop("ISC evaluation not allowed in this function.")
  ...
  pwr <- .pwr.fwl.sds(design_dta=design_dta, ldiff=log(theta0), 
...
# working horse based on .pwr.SABE.sds
# --------------------------------------
.pwr.fwl.sds <- function(muR=log(10), design_dta, ldiff, s2WR, s2WT, C2, 
                          nsims, ln_lBEL, ln_uBEL, #regulator, 
                          alpha=0.05, SABE_test="ABEL",
                          setseed=TRUE, details=FALSE, progress=FALSE)
{
  ...
  # ABE  decision using the fixed widened limits
  BE_fwl <- (loCL >=  log(0.75)) & (upCL <= log(1/0.75))

  # pe constraint
  BE_pe <- (pes >= ln_lBEL & pes <= ln_uBEL)
  
  # mixed decision
  BE <- ifelse(s2wRs > s2switch, BE_fwl, BE_ABE)
  ...
  names(p) <- c("p(BE)", "p(BE-fwl)", "p(BE-pe)", "p(BE-ABE)")
  p[1] <- sum(BE)/nsims
  p[2] <- sum(BE_fwl)/nsims
  p[3] <- sum(BE_pe)/nsims
