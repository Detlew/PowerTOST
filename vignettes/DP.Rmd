...
```
| [Main Vignette](vignette.html) | [ABE](ABE.html) | [RSABE](RSABE.html) | [Non-Inferiority](NI.html) | <span title="You are here">Dose-Proportionality</span> | [Power Analysis](PA.html) |
|:-:|:-:|:-:|:-:|:-:|:-:|
```{r setup}
...
With `sampleN.dp(..., details = FALSE, print = FALSE)` results are provided as a data.frame with ten (if `design="crossover"`) or eleven (if `design="crossover"` or `design="IBD"`) columns: `Design`, `alpha`, `CV`, [`CVb`], `doses`, `beta0`, `theta1`, `theta2`, `Sample size`, `Achieved power`, and `Target power`. To access *e.g.*, the sample size use `sampleN.dp[["Sample size"]]`.
...
# *Cave!*
There is no guarantee that a desired incomplete block design (for given dose level and number of periods) can be constructed. If you provide your own design matrix (in the argument `dm`) it is not assessed for meaningfulness.
