...
<th><span  style="color:#006400" title="You are here">Dose-Proportionality</span></th>
...
Rather extreme: Five levels, but we want only three periods. Hence, we opt for an incomplete block design. The design matrix is obtained by the function `bib.CL()`.
```{r}
levels <- 5
doses  <- mod.fibo(lowest, levels)
dm     <- bib.CL(trt = levels, p = 3)
...
Note that function `bib.CL()` supports only up to five levels and four periods. If you need more, use\
```
dm <- crossdes::des.MOLS(trt = levels, k = periods)
```
...
Man-pages of functions used in this vignette:
  ...
  * [bib.CL](library/html/bib.CL.html)
