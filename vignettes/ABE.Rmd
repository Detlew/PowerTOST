...
```
| [Main Vignette](library/doc/vignette.html) | <span title="You are here">ABE</span> | [RSABE](library/doc/RSABE.html) | [Non-Inferiority](library/doc/NI.html) | [Dose-Proportionality](library/doc/DP.html) | [Power Analysis](library/doc/PA.html) |
|:-:|:-:|:-:|:-:|:-:|:-:|
```{r setup}
...
With `sampleN.TOST(..., details = FALSE, print = FALSE)` results are provided as a data.frame with eight columns `Design`, `alpha`,  `CV`, `theta0`, `theta1`, `theta2`, `Sample size`, `Achieved power`, and `Target power`. To access *e.g.*, the sample size use either `foo[7]` or `foo[["Sample size"]]`. We suggest to use the latter in your code for clarity.
...
![**Fig. 1** Power curve ($\alpha$ 0.05, 2×2×2 design, *CV* 0.20)](figure3.png){width=358px}
...
> `r tufte::quote_footer('--- Donald J. Schuirmann, *[Two at a Time? Or All at Once?](https://www.enar.org/meetings/meetings2004/enar_final_program_2004.pdf#page=204)*')`
  ...
  * **Two at a Time**
    ...
    Say, you have one test (`A`) and two references from different regions (`B`, `C`). You get two separate within-subject variances (in the comparisons `A` *vs* `C` and `B` *vs* `C`).\
...
print(res, row.names = FALSE)
```
Four treatments (*e.g.*, T fasting/fed, R fasting/fed).
```{r}
CV  <- 0.20
...
In any case, calculation of *post hoc* (a.k.a. *a posteriori*, retrospective) power is futile.^[Hoenig JM, Heisey DM. [*The Abuse of Power: The Pervasive Fallacy of Power Calculations for Data Analysis.*](https://www.vims.edu/people/hoenig_jm/pubs/hoenig2.pdf) Am Stat. 2001; 55(1): 19--24]
...
This is an apples-and-oranges comparison. Red squares show *CV*s which were above the upper CL of the pooled *CV*. Given, only in three studies the lower CL did not overlapp with the upper one of the pooled *CV*.

> Which sided of the great divide are you on? Do you believe that meta is better or do you hold instead that pooling is fooling? Well, to nail my colours to the mast, I belong to the former school. It seems to me that there is no other topic in medical statistics, with the possible exceptions of cross-over trials, bioequivalence and n-of-1 studies, which has the same capacity as this one to rot the brains.
...
```
![**Fig. 4** Power Analysis (in each panel one argument is varied and others kept constant)](figure6.png){width=469px}

This excercise confirms what we already know. The most critical parameter is $\theta_{0}$, whereas dropouts are the least important.

More details are given in the respective [vignette](library/doc/PA.html).
...
```{r}
CVfromCI(lower = 0.8323, upper = 1.0392, n = 26,
         design = "2x2x4")
```
...
```{r}
CVfromCI(lower = 0.8323, upper = 1.0392, n = c(14, 12),
         design = "2x2x4")
```
The true value is lower. That means, if we use the *CV* assuming balanced sequences our sample size estimations will be conservative.
