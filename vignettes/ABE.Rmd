...
Implemented exact algorithms are `"exact"` / `"owenq"` (Owen’s Q function, default)^[Owen DB. *A special case of a bivariate non-central t-distribution.* Biometrika. 1965: 52(3/4); 437--46. [doi:10.2307/2333696](https://doi.org/10.2307/2333696).] and `"mvt"` (direct inegration of the bivariate non-central *t*-distribution). Approximations are `"noncentral"` / `"nct"` (non-central *t*-distribution) and `"shifted"` / `"central"` (‘shifted’ central *t*-distribution).
...
Let us first recap the hypotheses in bioequivalence.
  ...
  2. The confidence interval inclusion approach (preferred in regulatory guidelines)
...
## Dropouts
As we have seen, the impact of dropouts on power is rather limited. Regularly CROs suggest additional subjects to ‘compensate for the potential loss in power’. IMHO, milking sponsors to make wealthy CROs richer. Note that the dropout-rate is based on *dosed* subjects. Hence, the correct formula for the adjusted sample size $\small{n{}'}$ based on the estimated one $\small{n}$ is $\small{n{}'=n / (1-{dropout\:rate})}$, and <u>not</u> $\small{n{}'=n \times (1+{dropout\:rate})}$. An example for studies with three periods:
```{r DO}
balance <- function(x, y) {
  x <- ceiling(x) + ceiling(x) %% y
  return(x)
}
...
```
The method is exact if the subjects/sequence are known. In the literature commonly only the total sample size is given and the function tries to keep sequences as balanced as possible. What if the study was imbalanced?\
A total sample size of 26 was reported. The study was either balanced or imbalanced to an unknown degree:
```{r lit2}
n     <- 26
n1    <- balance(seq(n, 16, -1), 2) / 2
n2    <- n - n1
