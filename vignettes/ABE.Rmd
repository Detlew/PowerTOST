...
```
| [Main Vignette](vignette.html) | <span style="color:#006400" title="You are here">ABE</span> | [RSABE](RSABE.html "Reference-scaled Average Bioequivalence") | [Non-Inferiority](NI.html) | [Dose-Proportionality](DP.html) | [Power Analysis](PA.html) |
|:-:|:-:|:-:|:-:|:-:|:-:|
...
`"robust=TRUE"` forces `n-seq` degrees of freedom and is used only in higher-order crossover designs. Could be used if the evaluation was done with a mixed-effects model.

With `sampleN.TOST(..., details = FALSE, print = FALSE)` results are provided as a data.frame with nine columns `Design`, `alpha`,  `CV`, `theta0`, `theta1`, `theta2`, `Sample size`, `Achieved power`, and `Target power`. To access *e.g.*, the sample size use either `foo[1, 7]` or `foo[["Sample size"]]`. We suggest to use the latter in your code for clarity.
...
CVfromCI(lower = 0.8323, upper = 1.0392, n = c(14, 12),
         design = "2x2x4")
```
The true value is lower. That means, if we use the *CV* -- falsely -- assuming balanced sequences our sample size estimations will be conservative.
...
# License
<h4>Helmut Schütz `r Sys.Date()`</h4>
