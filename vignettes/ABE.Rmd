...
With `sampleN.TOST(..., details = FALSE, print = FALSE)` results are given as a data.frame with eight columns `Design`, `alpha`,  `CV`, `theta0`, `theta1`, `theta2`, `Sample size`, `Achieved power`, and `Target power`. To access *e.g.*, the sample size use either `foo[7]` or `foo[["Sample size"]]`. We suggest to use the latter in your code for clarity.
...
# Higher-order studies{#higher}
Designs with three and four periods are supported.
```{r, echo = FALSE}
res        <- data.frame(design = rep(NA, 3), name = NA, df = NA)
...
for (i in 1:2) {
  res[i, 2:3] <- signif(sampleN.TOST(CV = CV, design = res$design[i],
...
# Hints{#hints}
## Power Analysis{#pa}
Although we suggest to explore the various options shown in [Example 3](#expl3), a first look with the function `pa.ABE()` is worthwhile.
```{r}
pa.ABE(CV = 0.20, theta0 = 0.92)
```
![**Fig. 4** Power Analysis (in each pannel one argument is varied and others kept constant)](figure6.png){width=469px}
...
invisible(ifelse(res$el2 - n >=0, res$opt <- res$el2, res$opt <- res$el1))
