...
| $\small{\theta_{0}}$ | `theta0` | ‘True’ or assumed T/R ratio | see below |
| *CV* | `CV` | CV | none |
...
* `logscale = TRUE`
$$\small{H_{0}:\theta_0 \leq \log({margin})\:vs\:H_{1}:\theta_0>\log({margin})}$$
    where $\small{\theta_0=\mu_T/\mu_R}$

* `logscale = FALSE`
$$\small{H_{0}:\theta_0 \leq {margin}\:vs\:H_{1}:\theta_0>{margin}}$$
